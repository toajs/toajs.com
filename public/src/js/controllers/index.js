define(function(require, exports, module) {
  ...
  AnimationGsap = require('animation-gsap');
  ...
  var PageIndex = (function() {
    ...
    pageIndex.prototype.npmInstall = function() {
      ...
      function runScripts(data, pos) {
        var $prompt = $('.prompt', this.$terminal),
            script = data[pos];
        if(script.clear === true) {
          $('.history', this.$terminal).html('');
        }
        switch(script.action) {
          case 'type':
            // cleanup for next execution
            $prompt.removeData();
            $('.typed-cursor', this.$terminal).text('');
            $prompt.typed({
              strings: script.strings,
              typeSpeed: 30,
              callback: function() {
                var history = $('.history', this.$terminal).html();
                history = history ? [history] : [];
                history.push('$ ' + $prompt.text());
                if(script.output) {
                  history.push(script.output);
                  $prompt.html('');
                  $('.history', this.$terminal).html(history.join('<br>'));
                }
